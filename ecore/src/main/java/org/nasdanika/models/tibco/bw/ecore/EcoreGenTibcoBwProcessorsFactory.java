package org.nasdanika.models.tibco.bw.ecore;
...
@EPackageNodeProcessorFactory(nsURI = BwPackage.eNS_URI)
public class EcoreGenTibcoBwProcessorsFactory {

	private Context context;
		
	public EcoreGenTibcoBwProcessorsFactory(Context context) {
		this.context = context;
	}
	
	@EPackageNodeProcessorFactory(
			label = "Tibco BW Model",
			actionPrototype = """
                    app-action:
                      content:
                        content-markdown:
		...
		return new EPackageNodeProcessor(config, context, prototypeProvider) {
			...
			protected void generateDiagramAndGraphActions(Collection<Label> labels, ProgressMonitor progressMonitor) {
				...
				try {
					...
					Files.writeString(new File("target/bw.drawio").toPath(), document.save(null));
				} catch (Exception e) {
					e.printStackTrace();
